...
export default function GameCategory() {
    ...
    const level = parseInt(localStorage.getItem(`${categoryCode}Level`) || "1");
    ...
    // Cargar datos de categoría desde BD para logo y color
    useEffect(() => {
    ...
    const nextCard = async (direction) => {
        if (currentCardIndex >= questions.length - 1) {
            const currentLevel = parseInt(localStorage.getItem(`${categoryCode}Level`) || "1");
            const nextLevel = currentLevel + 1;

            localStorage.setItem(`${categoryCode}Level`, nextLevel);
