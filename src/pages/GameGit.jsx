...
export default function GameGit() {
    ...
    const loadProgress = async () => {
      try {
        if (isAuthenticated && token) {
          ...
          const userLevel = progress.git?.level || 1;
          setLevel(userLevel);
        } else {
          // Modo invitado: usar localStorage
          const localLevel = parseInt(localStorage.getItem("gitLevel") || "1");
          setLevel(localLevel);
        }
      } catch (error) {
  ...
  const nextCard = async (direction) => {
    if (currentCardIndex >= totalCards - 1) {
      ...
      if (isAuthenticated && token) {
        try {
          await updateUserProgress(token, 'git', { current_level: newLevel });
        } catch (error) {
          console.error('Error updating progress:', error);
        }
      } else {
        localStorage.setItem("gitLevel", newLevel);
      }
